...
observe({
  ...
  max_date <- max(as.Date(data$Tiempo,format = "%Y-%m-%d"), na.rm = TRUE)
...
variations_df <- reactive({
  ...
  # Calculer le total global pour toutes les proportions
  total_global <- sum(summary_df$total_delta_kg, na.rm = TRUE)
  
  # Ajouter la proportion et trier les résultats
  summary_df <- summary_df %>%
    mutate(total_proporcion_delta = total_delta_kg / abs(total_global)) %>%
