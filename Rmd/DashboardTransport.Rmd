...
observe({
  ...
  min_date <- min(data$Tiempo, na.rm = TRUE)
  max_date <- max(data$Tiempo, na.rm = TRUE)
...
output$valueBox6 <- renderUI({
  ...
  div(
    ...
    div(
      h4("Distance Totale sur les 15 jours derniers jours"),
      h2(value)
    )
  )
})
...
output$valueBox7 <- renderUI({
  ...
  div(
    ...
    div(
      h4("Distance Totale dans la zone de temps sélectionnée"),
      h2(value)
    )
  )
})
...
output$valueBox8 <- renderUI({
  ...
  div(
    class = "custom-container",
    div(
      h4("Le temps passé en circulant sur la zone sélectionnée"),
      h2(value)
    )
  )
})
...
# Créer une liste d'options pour le pickerInput
...
# ------------------- Filtrage des Données -------------------
# Fonction réactive pour filtrer les données en fonction de la condition et des acheteurs sélectionnés
df_compra_filtered <- reactive({
  if ("Seleccionar todo" %in% input$ID006) {
    # Si "Seleccionar todo" est sélectionné, filtre uniquement par condition
    df_compra %>% filter(Condicion == input$ID005)
  } else {
    # Sinon, filtre par acheteurs sélectionnés et condition
...
output$valueBox1 <- renderUI({
  ...
  div(
    class ="custom-container",
    div(
      h4("Le prix au kilo d'un sac acheté en moyenne"),
      h2(value)
    )
  )
})
...
output$valueBox2 <- renderUI({
  ...
  div(
    class ="custom-container2",
    div(
      h4("Nombre d'achats par jours et par acheteurs"),
      h2(value)
    )
  )
})
...
output$valueBox3 <- renderUI({
  ...
  div(
    class ="custom-container",
    div(
      h4("Total des Écarts de Poids entre Achat et Réception à l'Usine"),
      h2(value)
    )
  )
})
...
output$valueBox4 <- renderUI({
  ...
  div(
    class ="custom-container2",
    div(
      h4("Moyenne des Variations de Poids"),
      h2(value)
    )
  )
})
...
output$valueBox5 <- renderUI({
  ...
  div(
    class ="custom-container",
    div(
      h4("Nombre de KG acheté (Brut arrivé à l'usine)"),
      h2(value)
    )
  )
})
...
# ------------------- Données Longues -------------------
# Transformation des données en format long et ajout de colonnes supplémentaires
data_long <- reactive({
  df_compra_filtered2() %>%
...
# ------------------- Tableau Résumé -------------------
# Création d'un tableau résumé avec les données filtrées
...
output$plot2 <- renderPlotly({
  p <- ggplot(data_long(), aes(x = Week_Start_Date, y = Kg, fill = Type, color = Type, group = Type)) +
    ...
    scale_fill_manual(values = c("total_kg_pagado" = "#E6B336", "total_kg_Bruto" = "#8B0000")) +
    scale_color_manual(values = c("total_kg_pagado" = "#E6B336", "total_kg_Bruto" = "#8B0000")) +
    scale_x_date(
      name = "Date",
      date_breaks = "1 month",
    ...
    theme(
      ...
      axis.text.x = element_text(angle = 45, hjust = 1, color = "#8B0000"),
      legend.position = "bottom",
      plot.title = element_text(size = 16, color = "#000000"),
      plot.title.position = "plot",  # Positionne le titre au-dessus du graphique
      axis.title = element_text(size = 10, color = "#000000"),
      axis.text = element_text(size = 10, color = "#000000"),
      panel.grid.minor = element_blank(),
      panel.grid.major = element_line(color = "#000000", size = 0.3)
    )
  ggplotly(p)
})
